...
func NewLoader(config LoaderConfig) (*Loader, error) {
	...
	if !info.IsDir() {
		return nil, fmt.Errorf(
			"knowledge root is not a directory: %s",
			config.Root,
		)
	}
...
func (l *Loader) sourceFor(path string) (string, error) {
	...
	if err != nil ||
		...
		return "", fmt.Errorf(
			"knowledge path %q is outside the configured root",
			path,
		)
	}

	if !utf8.ValidString(relative) ||
		containsControlCharacter(relative) {
		return "", fmt.Errorf(
			"knowledge path %q is not a valid document source",
			path,
		)
	}
...
func (l *Loader) verifyRoot() error {
	...
	if filepath.Clean(resolvedRoot) != l.root {
		return errors.New(
			"knowledge root changed after loader initialization",
		)
	}
