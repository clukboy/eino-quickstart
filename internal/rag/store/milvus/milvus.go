...
// CollectionStore 扩展 Store，增加集合名称获取能力
...
type MilvusConfig struct {
	...
	Dimensions int
	MetricType string
	Token      string // 新增: 支持 "user:password" 格式认证
	DBName     string // 新增: 支持多数据库
}
...
type milvusAPI interface {
	...
	Search(ctx context.Context, collection string, embedding []float32, limit int, metric entity.MetricType) ([]SearchResult, error)
...
// NewMilvusStore 创建 Milvus 向量存储实例
func NewMilvusStore(ctx context.Context, config *MilvusConfig) (*MilvusStore, error) {
	...
	if config.Token != "" {
		clientConfig.Username, clientConfig.Password, _ = strings.Cut(config.Token, ":")
	}
...
// EnsureCollection 确保集合存在、Schema 正确且已加载
func (s *MilvusStore) EnsureCollection(ctx context.Context) error {
	...
	if !exists {
		...
		if err := s.client.CreateCollection(ctx, s.collection, schema, s.metricType); err != nil {
...
func (s *MilvusStore) Upsert(ctx context.Context, chunkIDs []int64, vectors [][]float32) error {
	if err := s.validate(); err != nil {
		return err
	}
	if len(vectors) == 0 {
	...
	if err := s.client.Upsert(ctx, s.collection, chunkIDs, vectors, s.dimensions); err != nil {
...
func (s *MilvusStore) Search(ctx context.Context, embedding []float32, limit int) ([]SearchResult, error) {
	if err := s.validate(); err != nil {
		return nil, err
	}
	if limit <= 0 {
	...
	results, err := s.client.Search(ctx, s.collection, append([]float32(nil), embedding...), limit, s.metricType)
	if err != nil {
...
func validateCollectionSchema(collection *entity.Collection, dimensions int) error {
	...
	if embeddingField == nil || embeddingField.DataType != entity.FieldTypeFloatVector {
...
func validateVector(values []float32, dimensions int) error {
	if len(values) != dimensions {
		return fmt.Errorf("dimension %d does not match configured dimension %d", len(values), dimensions)
	}
...
// ==================== Milvus Client Adapter 实现 ====================

func (c *milvusClientAdapter) HasCollection(ctx context.Context, collection string) (bool, error) {
	return c.client.HasCollection(ctx, milvusclient.NewHasCollectionOption(collection))
}

func (c *milvusClientAdapter) DescribeCollection(ctx context.Context, collection string) (*entity.Collection, error) {
	return c.client.DescribeCollection(ctx, milvusclient.NewDescribeCollectionOption(collection))
}

func (c *milvusClientAdapter) CreateCollection(ctx context.Context, collection string, schema *entity.Schema, metric entity.MetricType) error {
	if err := c.client.CreateCollection(ctx, milvusclient.NewCreateCollectionOption(collection, schema)); err != nil {
		return err
	}
...
func (c *milvusClientAdapter) LoadCollection(ctx context.Context, collection string) error {
	task, err := c.client.LoadCollection(ctx, milvusclient.NewLoadCollectionOption(collection))
	if err != nil {
		return err
	}
...
func (c *milvusClientAdapter) Upsert(ctx context.Context, collection string, ids []int64, vectors [][]float32, dimensions int) error {
	_, err := c.client.Upsert(ctx, milvusclient.NewColumnBasedInsertOption(collection).
...
func (c *milvusClientAdapter) Delete(ctx context.Context, collection string, ids []int64) error {
	_, err := c.client.Delete(ctx, milvusclient.NewDeleteOption(collection).WithInt64IDs(milvusIDField, ids))
	return err
}

func (c *milvusClientAdapter) Search(ctx context.Context, collection string, embedding []float32, limit int, metric entity.MetricType) ([]SearchResult, error) {
	searchOpt := milvusclient.NewSearchOption(collection, limit, []entity.Vector{entity.FloatVector(embedding)}).
		WithANNSField(milvusEmbeddingField).
		WithOutputFields(milvusIDField)
	...
	if len(resultSets) != 1 {
		return nil, fmt.Errorf("Milvus search returned %d result sets for one query", len(resultSets))
	}
...
func (c *milvusClientAdapter) Ready(ctx context.Context, collection string) error {
	...
	state, err := c.client.GetLoadState(ctx, milvusclient.NewGetLoadStateOption(collection))
	if err != nil {
		return err
	}
